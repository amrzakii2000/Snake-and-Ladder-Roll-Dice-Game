...
Output::Output()
{
	...
	//Wallets of Players
	UI.PlayerWallets[0] = 100;
	UI.PlayerWallets[1] = 100;
	UI.PlayerWallets[2] = 100;
	UI.PlayerWallets[3] = 100;
	...
	//Info of Players
	UI.Info = "P0(" + to_string(UI.PlayerWallets[0]) + "," + to_string(UI.PlayerMoves[0]) + ")" +
		"P1(" + to_string(UI.PlayerWallets[1]) + "," + to_string(UI.PlayerMoves[1]) + ")"
		+ "P2(" + to_string(UI.PlayerWallets[2]) + "," + to_string(UI.PlayerMoves[2]) + ")"
		+ "P3(" + to_string(UI.PlayerWallets[3]) + "," + to_string(UI.PlayerMoves[3]) + ")" + " | Curr=" + to_string(UI.PlayerMoves[0]);
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int xSnakeHead[4] = { x12,x12 - xChange,x12,x12 + xChange };
	int ySnakeHead[4] = { y1,y1 + yChange,y1 + yChange * 2,y1 + yChange };
	...
	pWind->DrawPolygon(xSnakeHead, ySnakeHead, 4);
