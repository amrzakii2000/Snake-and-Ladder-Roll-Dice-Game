...
bool CardThirteen::flag_P_F = false;
bool CardThirteen::flag_owner = false;
Player* CardThirteen::owner = NULL;
Player* CardThirteen::Passer = NULL;
...
void CardThirteen::ReadCardParameters(Grid* pGrid)
{
	...
	{
		pOut->PrintMessage("New Card 13: Enter its card price ...");
		bool z1, z2;
...
void CardThirteen::SetFlagpF(bool pf)
{
	flag_P_F = pf;
}
bool CardThirteen::SetPrice(int p)
{
	if (p > 0)
	{
		price = p;
...
bool CardThirteen::SetFees(int f)
{
	if (f > 0)
	{
		fees = f;
