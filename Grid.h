...
public:
	...
	bool AddObjectToCell(GameObject* pNewObject, CellPosition pos2);
	...
	void UpdatePlayerCell(Player * player, const CellPosition & newPosition); // Update the player's pCell with the CellList's Cell pointer of the "newPosition",
	 ...
	 bool  IsOverLapping(GameObject* pNew) const;												// and  Draws it in the new cell
	
	 void EditableCard(Card*& pCard,CellPosition& cardPosition);
	...
	void SetClipboard(CellPosition pos);
	...
	Player* GetPlayerfromNum(int playerNum); //Gets a Pointer tha a player with passed Num
	...
	void ResetGame();					//Used for New Game ction to reset Player wallets, turncounts and positions
	void DataToSave(Ladder** l, Snake** s, Card** c, int&, int&, int&); // A function used for saving the grid
